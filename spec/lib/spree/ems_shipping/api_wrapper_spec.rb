...
describe "Ems::ApiWrapper" do
  ...
  before :each do
    @wrapper = Ems::ApiWrapper.new(Ems::Config[:api_url])
  end

  describe "ApiWrapper::normalize_location_name" do
    it "returns empty string for nil" do
      Ems::ApiWrapper::normalize_location_name(nil).should be_empty
    end
    example "non unicode" do
      name = '  TverSkoy   RegiOn  '
      normalized_name = 'TVERSKOY REGION'
      (Ems::ApiWrapper::normalize_location_name(name) == normalized_name).should be_true
    end
  ...
  describe "error_wrap" do
    it "raise Ems::ApiWrapperError if something wrong with api" do
      expect { Ems::ApiWrapper.new(nil).max_weight }.to raise_error(Ems::ApiWrapperError)
    end
  end
end